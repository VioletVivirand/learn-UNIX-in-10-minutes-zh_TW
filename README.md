...
**Bash** 和 **tcsh**（其他 shell 也可能有）的特殊功能：您可以按下 `TAB` 鍵來補齊已經漸入了一部份的檔名。舉例來說：如果您想要編輯一個在目錄裡且叫做 `constantine-monks-and-willy-wonka.txt` 的檔案，您可以鍵入 `vi const`，再按下 `TAB` 鍵，這時 shell 會補齊剩下的文字（不過這個檔名必須是獨一無二的
）。
